...
void processShell()
{
    ...
    if (kbhitUart0())
    {
        ...
        else
        {
            ...
            if (strcmp(token, "vote") == 0)
            {
                token = strtok(NULL, " ");
                if (token != NULL)
                {
                    uint8_t songNum = asciiToUint8(token);
                    castVote(songNum);
                }
                else
                    putsUart0("Usage: vote 1|2|3\n");
            }
            if (strcmp(token, "music") == 0)
            {
                ...
                else if (token != NULL && strcmp(token, "reset") == 0)
                {
                    ...
                    putsUart0("Usage: music status|reset\n");
            }
...
int main(void)
{
    ...
    // Init music voting system
    initMusic();

    // Init HTTP web server on port 80
    initHttp();

    // Init ethernet interface (eth0)
    putsUart0("\nStarting eth0\n");
    initEther(ETHER_UNICAST | ETHER_BROADCAST | ETHER_HALFDUPLEX);
    setEtherMacAddress(2, 3, 4, 5, 6, 0x82); // 130 decimal = 0x82 hex
            ...
            if (isIp(data))
            {
            	if (isIpUnicast(data))
            	{
                    ...
                    if (isTcp(data))
                    {
                        ...
                        {
                            // Route TCP packets to the correct handler
                            // based on the destination port
                            ipHeader *ipTcp = (ipHeader*)data->data;
                            tcpHeader *tcpPkt = (tcpHeader*)((uint8_t*)ipTcp + (ipTcp->size * 4));
                            uint16_t dstPort = ntohs(tcpPkt->destPort);

                            socket *ms = getMqttSocket();
                            if (ms->state != TCP_CLOSED && dstPort == ms->localPort)
                            {
                                // Packet is for the MQTT client connection
                                processTcpResponse(data);
                            }
                            else if (dstPort == HTTP_PORT)
                            {
                                // Packet is for the HTTP web server
                                processHttpTcpPacket(data);
                            }
                        }
                        else
                ...
                if (isUdp(data))
                {
                    if (isDhcpResponse(data))
                    {
                        ...
                        /* skip generic UDP handling - don't send "Received" to DHCP server */
                    }
                    else
                    {
